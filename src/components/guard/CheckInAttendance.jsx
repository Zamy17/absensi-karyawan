// src/components/guard/CheckInAttendance.jsx
import React, { useState, useEffect } from 'react';
import { useAuth } from '../../contexts/AuthContext';
import { getEmployees, getAttendanceByDate, recordCheckIn } from '../../firebase/firestore';
import { getCurrentDate, getCurrentTime, isWithinCheckInHours, isWorkday } from '../../utils/dateUtils';
...
const CheckInAttendance = () => {
  ...
  // Check if today is a workday and within check-in hours
  const canRecordAttendance = isWorkday() && isWithinCheckInHours();
  ...
  const handleConfirmCheckIn = async () => {
    try {
      ...
      setTodayAttendance([...todayAttendance, result]);
  ...
  const getEmployeeAttendanceStatus = (employeeId) => {
    ...
    if (!record) {
      ...
      return `Masuk: ${record.checkInTime} (${record.status})`;
    } else {
      return "Belum Absen";
    }
  };
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-gray-500">
            Konfirmasi absensi masuk karyawan (06:00 - 09:00)
          </p>
        </div>
              ...
              <div className="ml-3">
                <p className="text-sm text-yellow-700">
                  {!isWorkday() 
                    ...
                    : "Saat ini di luar jam absensi masuk (06:00 - 09:00)."}
                </p>
              </div>
            </div>
          </div>
